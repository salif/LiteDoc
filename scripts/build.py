...
def resolve_file_order(src: Path, preferred: list[str], glob: str) -> list[str]:
    """
    ...
    """
    existing_preferred = [p for p in preferred if (src / p).exists()]
...
def build():
    ...
    # Tesseract — prepend library + expose worker/core data URIs for ocr.js
    tess_lib   = safe_js(fetch_url("https://cdn.jsdelivr.net/npm/tesseract.js@5/dist/tesseract.min.js"))
    tess_wkr   = fetch_url("https://cdn.jsdelivr.net/npm/tesseract.js@5/dist/worker.min.js")
    tess_core  = fetch_url("https://cdn.jsdelivr.net/npm/tesseract.js-core@5/tesseract-core.wasm.js")

    if tess_lib:
        wkr_uri  = f"data:application/javascript;base64,{base64.b64encode(tess_wkr.encode()).decode()}"  if tess_wkr  else ""
        core_uri = f"data:application/javascript;base64,{base64.b64encode(tess_core.encode()).decode()}" if tess_core else ""
        # Expose as globals so ocr.js can pass them into Tesseract.createWorker()
        tess_globals = (
            f'window.__TESS_WORKER_URI = "{wkr_uri}";\n'
            f'window.__TESS_CORE_URI   = "{core_uri}";\n'
        )
        js_blob = tess_globals + tess_lib + "\n\n" + js_blob
        log(OK, "Tesseract inlined" + (" (worker+core as data URIs)" if wkr_uri else " (worker/core fetch at runtime)"))
    else:
        log(WARN, "Tesseract download failed — OCR will need network access")
    ...
    print("── Done ─────────────────────────────────")
    log(OK, f"output: {OUT_FILE.resolve()}")
